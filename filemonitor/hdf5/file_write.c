...
//system("hdparm -W 0 /media/volume/sdb");  // disable cache
...
int main(int argc, char *argv[])
{
   ...
   //collect output in a file
    ...
    char filename[100];
    char filename1[10];
    strcpy(filename,"output_blockchain/write/test2/write2_");
   ...
   // printf("row by rank %d\n",rows);
    ...
    for (i=0; i<rows; i++) {
        for(j=0;j<cols;j++){
            write_buf[i*cols+j] = ((mpi_rank*rows*cols)+(i*cols+j))%INT_MAX;
        } 
    }
