...
int main(int argc, char *argv[])
{
    ...
    int i; 
    int rows,cols; 
    int count,size;                    /* Loop index */ 
    
    hsize_t     X = 256;  // X is a constant. int is 4 bytes. so 4 bytes*256=1024B=1KB
    // hsize_t     X = 2;  // X is a constant. int is 4 bytes. so 4 bytes*256=1024B=1KB
   ...
   // printf("row by rank %d\n",rows_rank);
    ...
    file_start[0] = mpi_rank*rows_rank;
    file_start[1] = 0;
    file_count[0] = rows_rank;
    file_count[1] = cols;
    ...
    write_buf = (int *)malloc(sizeof(int)*cols*rows_rank);
    for (i=0; i<cols*rows_rank; i++) {
        write_buf[i] = i;
    }
    
  //printf("cols*rows_rank=%d\n",cols*rows_rank);
    ...
    if (mpi_rank == 0) {
        ...
        printf(" count=%d size=%d Runtime is  %f \n",count,size, duration);
