...
sys.path.append('/home/exouser/filemonitor/filemonitor/grpc/examples/python/helloworld/')
...
def check_mode(filename, mode,comm,pid,CPU,uid,timestamp):
    ...
    if((filename not in mod_dict)):
        ...
        # elif(mod_dict[filename]['timestamp']!=timestamp):
            ...
            updated_query={filename:{'program':comm,'mode':mode,'pid':pid,'username':username,'timestamp':timestamp}}
            # print("updated query is: ")
            # print("updated query is: ", json.dumps(updated_query))
            mod_dict.update(json.dumps(updated_query))
            print ("change mode")
            print("updated query is: ", json.dumps(updated_query))
            # print(mod_dict)
            return 1
...
def submitTransaction(filename, mode,pid,username,timestamp,cpu,operation):
    ...
    print(str)
    # cmd='source /home/exouser/grpc_python/venv/bin/activate'
    # os.system(cmd)
    # cmd='cd ~/grpc_python/grpc/examples/python/helloworld'
    # os.system(cmd)
    # os.chdir('grpc/examples/python/helloworld/')
    
    cmd ='python3 grpc/examples/python/helloworld/greeter_client.py'
