...
def submitTransaction(filename, mode,pid,username,timestamp):
    ...
    try:
        ...
        def print_event(cpu, data, size):
            ...
            if(return_val==1):
                ...
                # os.system("echo 'test'")
                if(event.pid not in process_id):
                    # print("duplicate\n")
                    submitTransaction(filename, mode1,event.pid,username1,timestamp1)
                    process_id.append(event.pid)
                


# # print("Integer timestamp of current datetime: ",
                    # timestamp)
                # username = uid_to_username(event.uid)
                # if username:
                #     print(f"Username for UID {event.uid}: {username}")
                # else:
